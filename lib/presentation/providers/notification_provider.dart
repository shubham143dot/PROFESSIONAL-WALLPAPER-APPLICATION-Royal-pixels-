...
class NotificationNotifier extends AsyncNotifier<List<NotificationItem>> {
  ...
  Future<List<NotificationItem>> build() async {
    ...
    final String? userId =
        authState.isAuthenticated ? authState.user?.uid : null;
    ...
    _broadcastSubscription =
        repository.getBroadcastNotifications().listen((broadcasts) async {
  ...
  Future<void> _processBroadcasts(List<NotificationItem> broadcasts) async {
    ...
    final List<String> processedIds =
        prefs.getStringList(_processedBroadcastsKey) ?? [];
    bool updated = false;
