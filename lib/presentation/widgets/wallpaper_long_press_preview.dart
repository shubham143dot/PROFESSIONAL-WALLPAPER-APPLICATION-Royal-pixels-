...
class _WallpaperLongPressOverlayState
  ...
  void initState() {
    ...
    _blur = Tween<double>(begin: 0, end: 10).animate(
      CurvedAnimation(
          parent: _ctrl, curve: const Interval(0, 0.70, curve: Curves.easeOut)),
    );
    _dim = Tween<double>(begin: 0, end: 0.70).animate(
      CurvedAnimation(
          parent: _ctrl, curve: const Interval(0, 0.70, curve: Curves.easeOut)),
    );
    ...
    _cardScale = Tween<double>(begin: 1.0, end: 1.09).animate(
      CurvedAnimation(
          parent: _ctrl,
          curve: const Interval(0, 0.80, curve: Curves.fastOutSlowIn)),
    );
  ...
  Widget build(BuildContext context) {
    ...
    // Extract heavy static content so it doesn't rebuild every 16ms during animation
    final cardContent = Stack(
      fit: StackFit.expand,
      children: [
        // Image — uses mediumUrl (600px) for faster preview
        CachedNetworkImage(
          imageUrl: widget.wallpaper.mediumUrl,
          fit: BoxFit.cover,
          // memCacheHeight removed to ensure instant preview
    ...
    final actionPanel = _ActionPanel(
      ...
      onGoToFeed: () async {
        await _dismiss();
        if (context.mounted) {
          ref.read(navigationProvider.notifier).goToFeeds(
...
class _ActionPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final panelContent = Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.bg0.withValues(alpha: AdaptivePerformance.enableBackdropBlur ? 0.85 : 0.98),
