...
class UploadCategoryCoverPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<UploadCategoryCoverPage> createState() =>
      _UploadCategoryCoverPageState();
}

class _UploadCategoryCoverPageState
    extends ConsumerState<UploadCategoryCoverPage> {
  File? _selectedImage;
        ...
        if (fileSizeInMb > 2) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    'Cover size must be under 2 MB. Selected file is ${fileSizeInMb.toStringAsFixed(2)} MB.'),
    ...
    SafeTap.run('admin_upload_cover', () async {
      ...
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text('Overrode $_selectedCategory cover successfully! 🚀')),
        );
        context.pop(); // Go back
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
