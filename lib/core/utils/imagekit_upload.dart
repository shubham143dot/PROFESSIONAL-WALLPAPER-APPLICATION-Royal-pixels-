...
class ImageKitUpload {
  ...
  static const String _ikUploadUrl   = 'https://upload.imagekit.io/api/v1/files/upload';
  ...
  static Future<String?> uploadImage(
    ...
    // Free: ImageKit first, Cloudinary free as fallback.
    if (kDebugMode) debugPrint('[Upload] Free → trying ImageKit...');
    final ikUrl = await _uploadToImageKit(imageFile);
    ...
    if (kDebugMode) {
      debugPrint('[Upload] ImageKit failed — falling back to Cloudinary free...');
    }
    ...
    if (clUrl != null) {
      if (kDebugMode) debugPrint('[Upload] Cloudinary fallback succeeded ✓ → $clUrl');
    } else {
      if (kDebugMode) debugPrint('[Upload] Both ImageKit and Cloudinary failed ✗');
    }
    return clUrl;
  }
